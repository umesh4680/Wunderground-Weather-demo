...
const NSString *kWundergroundKey = @"8ad70ada48c4fba7";//@"dxxxxxxxxxxxxxxx";
...
- (void)viewDidLoad
{
   ...
   // To Show and hide Zip Code Buttons
     // [self hideZipCode:YES animate:NO];
    
      [self hideZipCode:NO animate:NO];
...
- (void)retrieveWeatherForLocation:(CLLocation *)location orZipCode:(NSString *)zipCode
{
    ...
    NSLog(@"weatherData = %@",[weatherData description] );

    NSLog(@"weatherResults = %@",[weatherResults description]);
    ...
    NSDictionary *displayLocationDict = currentObservation[@"display_location"];
    NSString *cityName = displayLocationDict[@"city"];
    [self.cityNameLabel setText:cityName];
    
    
    NSString *countryName = displayLocationDict[@"state_name"];
    [self.countryNameLabel setText:countryName];

    
    NSString *weaterType = currentObservation[@"icon"];
    [self.weatherTypeLabel setText:weaterType];
    
    
    
    NSString *weaterTypeImageUrl = currentObservation[@"icon_url"];
    [self.weatherTypeImageView setImage:[UIImage imageWithData:[NSData dataWithContentsOfURL:[NSURL URLWithString:weaterTypeImageUrl]]]];
